  ...
  methods: {
    ...
    updateParameters(name) {
      ...
      var modifyScale = function (){
        if(sca.fixed) self.alertsStore.showDanger('Scale is fixed')
      ...
      var modifyVote = function (){
        if(v.fixed) self.alertsStore.showDanger('Vote is fixed')
        ...
        if(v.value < Math.pow(10,v.min)) {
          ...
          if(rs0.fixed) self.alertsStore.showDanger('Rs0 is fixed')
      ...
      var modifyRs0 = function (){
        if(rs0.fixed) self.alertsStore.showDanger('Rshares0 is fixed')
        rs0.value = rs1.value - v.value
        self.calcSliderValues(irs0)
      }

      var modifyRst = function (){
        if(rst.fixed) self.alertsStore.showDanger('RsharesTotal is fixed')
        rst.value = sca.value * rs1.value
        self.calcSliderValues(irst)
      }

      var modifyRs1 = function(type){
        if(rs1.fixed) self.alertsStore.showDanger('Rshares1 is fixed')
        if(type === 'scale')
